...
void main() {
	...
	vec4 sand = (smoothstep(-0.01, 0.03, vAmount) - smoothstep(0.03, 0.037, vAmount)) * texture2D(sand, vUV * 20.0);
	vec4 grassy = (smoothstep(0.0, 0.14, vAmount) - smoothstep(0.15, 0.50, vAmount)) * texture2D( grass, vUV * 20.0 );
